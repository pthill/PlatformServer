	...
	if (m_bGameStarted==false)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if ((m_pGameServiceOption->lMinTableScore!=0L)&&(pIServerUserItem->GetUserScore()<m_pGameServiceOption->lMinTableScore))
			{
				...
				{
					_sntprintf(szDescribe,CountArray(szDescribe),TEXT("您的游戏币少于 ") SCORE_STRING_EX TEXT("，不能继续游戏！"), AddDecimal(m_pGameServiceOption->lMinTableScore) );
				}
				else
				{
					_sntprintf(szDescribe,CountArray(szDescribe),TEXT("您的游戏积分少于 ") SCORE_STRING_EX TEXT("，不能继续游戏！"), AddDecimal(m_pGameServiceOption->lMinTableScore) );
				}
...
bool CTableFrame::PerformStandUpAction(IServerUserItem * pIServerUserItem, bool bInitiative/*=false*/)
{
	...
	if ((m_pGameServiceOption->wServerType==GAME_GENRE_MATCH) && (m_pGameMatchOption->cbMatchType==MATCH_TYPE_IMMEDIATE))
	{
		...
		//判断状态
		if (bInitiative==false && (cbUserStatus!=US_LOOKON && cbSignUpStatus==MUS_MATCHING))
		{
			if (pIServerUserItem->IsTrusteeUser()==false)
			{
				pIServerUserItem->SetTrusteeUser(true);
	...
	if (pIServerUserItem==pITableUserItem)
	{
		...
		//用户状态
		pIServerUserItem->SetClientReady(false);
		pIServerUserItem->SetUserStatus((cbUserStatus==US_OFFLINE && (bImmediteMatch==false || (bImmediteMatch==true && cbMatchStatus != MUS_MATCHING)))?US_NULL:US_FREE,INVALID_TABLE,INVALID_CHAIR);
...
bool CTableFrame::EfficacyEnterTableScoreRule(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	if (((lMinTableScore!=0L)&&(lUserScore<lMinTableScore))||((lLessEnterScore!=0L)&&(lUserScore<lLessEnterScore)))
	{
		//构造信息
		TCHAR szDescribe[128]=TEXT("");
		SCORE lMaxScore = __max(lMinTableScore,lLessEnterScore);
		if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
			_sntprintf(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_STRING_EX TEXT(" 的游戏币，您的游戏币不够，不能加入！"), AddDecimal(lMaxScore) );
		else if(m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
			_sntprintf(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_STRING_EX TEXT(" 的比赛币，您的比赛币不够，不能加入！"), AddDecimal(lMaxScore) );
		else
			_sntprintf(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_STRING_EX TEXT(" 的游戏积分，您的积分不够，不能加入！"), AddDecimal(lMaxScore) );
...
// 添加小数点
CString CTableFrame::AddDecimal( LONGLONG lScore ,  bool bComma /*= true*/, bool bPlus /*= false*/)
{
	CString strScore;
	CString strReturn;
