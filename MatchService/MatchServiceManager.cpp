...
void *  CMatchServiceManager::QueryInterface(const IID & Guid, DWORD dwQueryVer)
{
	...
	QUERYINTERFACE(IMatchUserItemSink,Guid,dwQueryVer);	
	QUERYINTERFACE_IUNKNOWNEX(IMatchUserItemSink,Guid,dwQueryVer);
...
//用户退赛
bool  CMatchServiceManager::OnEventUserQuitMatch(IServerUserItem * pIServerUserItem, BYTE cbReason,WORD *pBestRank, DWORD dwSocketID)
{
	ASSERT(m_pIGameMatchItem!=NULL);
	if(m_pIGameMatchItem!=NULL)
	{
		return m_pIGameMatchItem->OnEventUserQuitMatch(pIServerUserItem,cbReason,pBestRank,dwSocketID);	
	}
...
//用户状态
bool CMatchServiceManager::OnEventMatchUserStatus(IServerUserItem * pIServerUserItem, BYTE cbOldUserStatus, BYTE cbCurrUserStatus)
{
	ASSERT(m_pIGameMatchItem!=NULL);
	if(m_pIGameMatchItem!=NULL)
	{
		return m_pIGameMatchItem->OnEventMatchUserStatus(pIServerUserItem,cbOldUserStatus,cbCurrUserStatus);	
	}
...
//用户接口
IUnknownEx * CMatchServiceManager::GetMatchUserItemSink()
{
	return QUERY_ME_INTERFACE(IMatchUserItemSink);
}
//////////////////////////////////////////////////////////////////////////////////
